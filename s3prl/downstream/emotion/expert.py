...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, downstream_expert, expdir, **kwargs):
        ...
        self.test_dataset = IEMOCAPDataset(
            DATA_ROOT, test_path, self.datarc["pre_load"]
        )
    ...
    def forward(self, mode, features, labels, filenames, records, **kwargs):
        ...
        records["predict"] += [
            self.test_dataset.idx2emotion[idx]
            for idx in predicted_classid.cpu().tolist()
        ]
        records["truth"] += [
            self.test_dataset.idx2emotion[idx] for idx in labels.cpu().tolist()
        ]
    ...
    def log_records(self, mode, records, logger, global_step, **kwargs):
        ...
        for key in ["acc", "loss"]:
            ...
            logger.add_scalar(
                f"emotion-{self.fold}/{mode}-{key}", average, global_step=global_step
            )
