...
class DownstreamExpert(nn.Module):
    def __init__(
        ...
        self.resample_ratio = (
            self.fs / self.datarc["fbank_config"]["n_shift"] * upstream_rate / FS
        )
    ...
    def get_dataloader(self, split):
        if split == "train":
            ...
            return self._get_eval_dataloader(
                CustomDataset(self.datarc["eval_list_file"])
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # padding
        input_feature_lengths = torch.IntTensor(
            [feature.shape[0] for feature in input_features]
        )
        input_features = pad_sequence(input_features, batch_first=True).to(
            device=device
        )
        ...
        if split == "custom_test":
            ...
            records["feature_lengths"] += (
                predicted_feature_lengths.cpu().numpy().tolist()
            )
            ...
            if split in ["dev", "test"]:
                ...
                records["predicted_features"] += (
                    predicted_features.cpu().numpy().tolist()
                )
                records["feature_lengths"] += (
                    predicted_feature_lengths.cpu().numpy().tolist()
                )
    ...
    def log_records(
        ...
        if split == "custom_test":
            ...
            for i, wav_path in enumerate(
                tqdm(records["wav_paths"], dynamic_ncols=True, desc="Saving files")
            ):
                ...
                hdf5_save_path = os.path.join(
                    hdf5_save_dir,
                    (self.trgspk + "_" + os.path.basename(wav_path)).replace(
                        ".wav", ".h5"
                    ),
                )
                ...
                wav_save_path = os.path.join(
                    wav_save_dir, self.trgspk + "_" + os.path.basename(wav_path)
                )
                write(
                    wav_save_path,
            ...
            if split in ["dev", "test"]:
                ...
                hdf5_save_dir = os.path.join(
                    self.expdir, str(global_step), split, "hdf5"
                )
                ...
                for i, wav_path in enumerate(tqdm(records["wav_paths"])):
                    ...
                    hdf5_save_path = os.path.join(
                        hdf5_save_dir,
                        "_".join(wav_path.split("/")[-2:]).replace(".wav", ".h5"),
                    )
                    write_hdf5(hdf5_save_path, "feats", fbank)
                    ...
                    # save generated waveform
                    wav_save_path = os.path.join(
                        wav_save_dir, "_".join(wav_path.split("/")[-2:])
                    )
                    write(
                        wav_save_path,
                        self.datarc["fbank_config"]["fs"],
